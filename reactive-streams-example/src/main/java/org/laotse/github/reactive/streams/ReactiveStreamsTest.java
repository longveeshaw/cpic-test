...
public class ReactiveStreamsTest {
	...
	/**
	 * AsyncSubscriber is an implementation of Reactive Streams `Subscriber`, it
	 * runs asynchronously (on an Executor), requests one element at a time, and
	 * invokes a user-defined method to process each element.
	 *
	 * NOTE: The code below uses a lot of try-catches to show the reader where
	 * exceptions can be expected, and where they are forbidden.
	 */
	public static abstract class AsyncSubscriber<T> implements Subscriber<T>, Runnable {

		// Signal represents the asynchronous protocol between the Publisher and
		// Subscriber
		...
		// Showcases a convenience method to idempotently marking the Subscriber as
		// "done", so we don't want to process more elements
		// herefor we also need to cancel our `Subscription`.
		private final void done() {
			// On this line we could add a guard against `!done`, but since rule 3.7 says
			// that `Subscription.cancel()` is idempotent, we don't need to.
			done = true; // If `whenNext` throws an exception, let's consider ourselves done (not
							// accepting more elements)
			if (subscription != null) { // If we are bailing out before we got a `Subscription` there's little need for
										// cancelling it.
				try {
					subscription.cancel(); // Cancel the subscription
				} catch (final Throwable t) {
					// Subscription.cancel is not allowed to throw an exception, according to rule
					// 3.15
		...
		// Returns whether more elements are desired or not, and if no more elements are
		// desired,
		// for convenience.
		...
		private final void handleOnSubscribe(final Subscription s) {
			if (s == null) {
			...
			} else if (subscription != null) { // If someone has made a mistake and added this Subscriber multiple
				...
				// We have to assign it locally before we use it, if we want to be a synchronous
				// `Subscriber`
				// Because according to rule 3.10, the Subscription is allowed to call `onNext`
				// synchronously from within `request`
				subscription = s;
				try {
					// If we want elements, according to rule 2.1 we need to call `request`
					// And, according to rule 3.2 we are allowed to call this synchronously from
					// within the `onSubscribe` method
		...
		private final void handleOnNext(final T element) {
			if (!done) { // If we aren't already done
				if (subscription == null) { // Technically this check is not needed, since we are expecting Publishers
					...
					try {
						if (whenNext(element)) {
							try {
								subscription.request(1); // Our Subscriber is unbuffered and modest, it requests one
															// element at a time
							} catch (final Throwable t) {
								// Subscription.request is not allowed to throw according to rule 3.16
						...
						try {
							...
							// Subscriber.onError is not allowed to throw an exception, according to rule
							// 2.13
		...
		// Here it is important that we do not violate 2.2 and 2.3 by calling methods on
		// the `Subscription` or `Publisher`
		private void handleOnComplete() {
			if (subscription == null) { // Technically this check is not needed, since we are expecting Publishers to
										// conform to the spec
				// Publisher is not allowed to signal onComplete before onSubscribe according to
				// rule 1.09
				(new IllegalStateException(
		...
		private void handleOnError(final Throwable error) {
			if (subscription == null) { // Technically this check is not needed, since we are expecting Publishers to
										// conform to the spec
				// Publisher is not allowed to signal onError before onSubscribe according to
				// rule 1.09
				(new IllegalStateException(
		...
		// We implement the OnX methods on `Subscriber` to send Signals that we will
		// process asycnhronously, but only one at a time

		@Override
		public final void onSubscribe(final Subscription s) {
			// As per rule 2.13, we need to throw a `java.lang.NullPointerException` if the
		...
		// This `ConcurrentLinkedQueue` will track signals that are sent to this
		// `Subscriber`, like `OnComplete` and `OnNext` ,
		// and obeying rule 2.11
		private final ConcurrentLinkedQueue<Signal> inboundSignals = new ConcurrentLinkedQueue<Signal>();

		// We are using this `AtomicBoolean` to make sure that this `Subscriber` doesn't
		...
		// This method makes sure that this `Subscriber` is only executing on one Thread
		// at a time
		private final void tryScheduleToExecute() {
			if (on.compareAndSet(false, true)) {
				try {
					executor.execute(this);
				} catch (Throwable t) { // If we can't run on the `Executor`, we need to fail gracefully and not violate
										// rule 2.13
					if (!done) {
						try {
							done(); // First of all, this failure is not recoverable, so we need to cancel our
									// subscription
						} finally {
							inboundSignals.clear(); // We're not going to need these anymore
							// This subscription is cancelled by now, but letting the Subscriber become
							// schedulable again means
